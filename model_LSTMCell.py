...
class SNLIAutoencoderCell(nn.Module):
    def __init__(self, config):
        super(SNLIAutoencoderCell, self).__init__()
        self.config = config
        ...
        # TODO: stack more LSTM cells 
        self.decoder = nn.LSTMCell(input_size=config.d_hidden, hidden_size=config.d_hidden)
    ...
    def forward(self, batch, target_input, mode):
        ...
        # TODO: remove [-1], it's a hack to use LSTMCell with one cell when there are more stacked LSTMs in encoder
        dec_h0 = self.linear_ht(torch.cat([ht_sent1, ht_sent2, one_hot_label], 2))[-1]
        dec_c0 = self.linear_ct(torch.cat([ct_sent1, ct_sent2, one_hot_label], 2))[-1]
        ...
        ct = dec_c0
        if mode == "for-output":
            dec_inp_t = target_input_embed[0, :, :].unsqueeze(0)
            for t in range(max_T_decoder):
                (ht, ct) = self.decoder(dec_inp_t, (ht, ct))
                out_t.append(self.output(ht).unsqueeze(0))
                i_t = torch.max(out_t[-1], 2)[1]
                dec_inp_t = self.projection(self.embed(i_t))
        elif mode == "for-output-TF":
            for t in range(max_T_decoder):
                dec_inp_t = target_input_embed[t, :, :].unsqueeze(0)
                (ht, ct) = self.decoder(dec_inp_t, (ht, ct))
                out_t.append(self.output(ht).unsqueeze(0))
        else:
            raise ValueError('The mode should be one of: "for-output", for-output-TF"')
